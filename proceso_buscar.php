...
{    
    ...
    if($ins["status"]=="ok")
    {   
        $query = "SELECT * FROM alumnos WHERE id_alumno LIKE '%".$alumno."%' OR nombre LIKE '%".$alumno."%' OR carrera LIKE '%".$alumno."%'";
        $res = mysqli_query($conexion,$query);
        
        if(!$res)
        {
            $error = "error al buscar alumno en BD";
            $mensaje["status"]=$error;
        }
        else
        {
            ...
            { 
                ...
                $userData[] = array('id_alumno'=> $id, 'nombre'=> $nombre, 'carrera'=> $carrera, 'seccion'=> $seccion,
                                        'fecha_ingreso'=> $fecha);
            }   
            $error = "ok";
            $mensaje["status"]=$error;
